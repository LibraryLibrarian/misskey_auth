...
このバージョンでは `flutter_secure_storage` を、10 系での移行を経ずに 9 系から 11 系へ更新します。Android で旧既定の暗号方式により保存された認証情報は直接引き継げず、対象アカウントごとに再認証が必要です。

- これは Android の保存データの互換性に関する破壊的変更です。iOS でも同様にデータが失われるという意味ではありません。
- 利用側のアプリで、認証情報の欠落と保存領域のエラーを処理してください。
