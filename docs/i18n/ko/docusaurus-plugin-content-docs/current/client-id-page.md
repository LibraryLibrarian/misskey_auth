...
- `client_id`는 `https://yoursite/yourapp/`와 같은 HTTPS URL이어야 합니다. Misskey 서버는 인가 과정에서 이 페이지를 가져옵니다.
...
- 인가 요청의 `redirect_uri`는 `<link>` 태그에 있는 URL과 스킴, 대소문자, 끝의 슬래시까지 완전히 일치해야 합니다. 두 위치에 같은 형식으로 작성하세요.
...
`h-app` 블록은 앱 이름을 Misskey에 전달합니다. Misskey는 인가 화면에 이 이름을 표시합니다.
...
- URL에 인가 코드가 포함됩니다. 이 페이지에서 서드파티 스크립트를 불러오지 마세요.

## 일반적인 오류

- **`Invalid redirect_uri`**: 요청의 `redirect_uri`가 client_id 페이지의 어떤 `<link rel="redirect_uri">`와도 완전히 일치하지 않습니다. 스킴, 도메인의 대소문자, 끝의 슬래시를 확인하세요.
