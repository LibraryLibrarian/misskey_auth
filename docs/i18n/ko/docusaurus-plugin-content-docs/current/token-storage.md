...
`MisskeyAuthManager.defaultInstance()`는 기본 시간 초과(연결 10초, 전송 및 수신 각각 20초)를 사용합니다. 변경하려면 `MisskeyAuthManager`를 직접 구성하세요. `MisskeyAuthManager`의 시간 초과는 자체 `/api/i` 요청에만 적용되므로 각 클라이언트에도 전달해야 합니다.
