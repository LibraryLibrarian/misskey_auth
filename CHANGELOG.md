...
- `SecureTokenStore.upsert` now updates the account index before writing the token, so a failed write can no longer leave a token that `list` and `clearAll` cannot find. If the token write fails, the account may appear in `list` without a stored token.
- `SecureTokenStore.clearAll` now deletes every key with the `misskey_token::` prefix instead of only the accounts in the index, so it also removes tokens left outside the index and still works when the index is corrupted. Keys that do not belong to this library are kept.
